...
const createSwapRequest = asyncHandler(async (req, res) => {
  ...
  if (!receiverId || !skillOfferedId || !skillRequestedId) {
    throw new ApiError(
      400,
      "Receiver, skill offered, and skill requested are required"
    );
  }
  ...
  if (!receiver.skillsOffered.includes(skillRequestedId)) {
    throw new ApiError(
      400,
      "Receiver doesn't have this skill in their offered skills"
    );
  }
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        populatedSwapRequest,
        "Swap request accepted and completed successfully"
      )
    );
});
...
const completeSwapRequest = asyncHandler(async (req, res) => {
  ...
  if (
    swapRequest.requester.toString() !== userId.toString() &&
    swapRequest.receiver.toString() !== userId.toString()
  ) {
    throw new ApiError(
      403,
      "You can only complete swap requests you're involved in"
    );
  }
...
const deleteSwapRequest = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, {}, "Swap request deleted successfully"));
});
...
const getSwapRequest = asyncHandler(async (req, res) => {
  ...
  if (
    swapRequest.requester._id.toString() !== userId.toString() &&
    swapRequest.receiver._id.toString() !== userId.toString() &&
    req.user.role !== "admin"
  ) {
    throw new ApiError(
      403,
      "You can only view swap requests you're involved in"
    );
  }
...
const getSwapRequestStats = asyncHandler(async (req, res) => {
  ...
  const rejectedRequests = await SwapRequest.countDocuments({
    status: "Rejected",
  });
  const cancelledRequests = await SwapRequest.countDocuments({
    status: "Cancelled",
  });
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        stats,
        "Swap request statistics retrieved successfully"
      )
    );
});
...
const getPublicSwapRequests = asyncHandler(async (req, res) => {
  ...
  if (skill) {
    query.$or = [{ skillOffered: skill }, { skillRequested: skill }];
  }
  if (search) {
