...
const createSkill = asyncHandler(async (req, res) => {
  ...
  const skill = await Skill.create({
    ...
    isGlobal:
      req.user?.role === "admin" ||
      req.user?.role === "000000000000000000000000",
  });
  ...
  return res
    .status(201)
    .json(new ApiResponse(201, populatedSkill, "Skill created successfully"));
});
...
const deleteSkill = asyncHandler(async (req, res) => {
  ...
  const usersWithSkill = await User.find({
    $or: [{ skillsOffered: skillId }, { skillsWanted: skillId }],
  });
...
const searchSkills = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, skills, "Skills found successfully"));
});
...
const approveSkill = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, updatedSkill, "Skill approved successfully"));
});
...
const rejectSkill = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, {}, "Skill rejected and deleted successfully"));
});
