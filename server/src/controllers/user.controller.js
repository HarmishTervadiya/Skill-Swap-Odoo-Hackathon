...
const updateAllSkills = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, updatedUser, "Skills updated successfully")
    );
});
...
const searchUsersBySkill = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, {
            ...
            currentPage: page,
            totalUsers,
            matchedSkills: skills.map(skill => ({ id: skill._id, name: skill.name })),
...
const toggleUserBan = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, updatedUser, `User ${isBanned ? 'banned' : 'unbanned'} successfully`)
    );
});
